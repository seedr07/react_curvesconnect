...
module.exports = C = {
  ...
  DropingMenu: createFactory
    show: ->
      $(@refs.drop.getDOMNode()).addClass('visible')
      $(@refs.dropContainer.getDOMNode()).css(left: 0)
    hide: ->
      $(@refs.drop.getDOMNode()).removeClass('visible')
    ignore: (e) ->
      e.preventDefault()
      e.stopPropagation()
    handleTouchMove: (e) ->
      ...
      if !@dirLock? and (deltaX != 0 or deltaY != 0)
        if Math.abs(deltaX) > Math.abs(deltaY)
          ...
          @dirLock = 'Y'
      if @dirLock == 'X'
        $(@refs.dropContainer.getDOMNode()).css(left: -deltaX)
      else if @dirLock == 'Y'
        $dropBody = $(@refs.dropBody.getDOMNode())
        $dropContainer = $(@refs.dropContainer.getDOMNode())
        @scrollTop = @lastScrollTop + deltaY
        maxScrollTop = $dropBody.outerHeight() - $dropContainer.innerHeight()
        @scrollTop = Math.max(@scrollTop, 0)
        @scrollTop = Math.min(@scrollTop, maxScrollTop)
        $dropBody.css(top: -@scrollTop)
    handleTouchStart: (e) ->
      @dirLock = null
      @startX = e.changedTouches[0].screenX
      @startY = e.changedTouches[0].screenY
    ...
    render: ->
      @startY = null
      D.div {ref: 'drop', className: 'droping-menu', onClick: @hide},
        D.div {ref: 'dropContainer', className: 'droping-menu-container', onClick: @ignore, onTouchMove: @handleTouchMove, onTouchStart: @handleTouchStart, onTouchEnd: @handleTouchEnd},
          D.div {ref: 'dropBody', className: 'droping-menu-body'},
            D.ul {},
              _.map(@props.children || [], (item) ->
                D.li {}, item
              )...
